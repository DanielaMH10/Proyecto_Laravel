...
<form action="" method="POST" id="invoiceform">
    ...
    <div class="row">
        ...
        <div class="col-sm-3">
            <input type="text" readonly class="form-control-plaintext totalProduct" id="totalProduct1"/></div>
    </div>
...
<script>
    ...
    function init(){
        ...
        productSelected = products.fliter( product => product.id == productId)
    ...
    addButton.addEventListener('click', (event) => {
        ...
        fila.innerHTML = `<div class="col-sm-3">
            <select name="product[]" id="product${contador}" class="form-select">
                @foreach ($products as $product)
                <option value="{{ $product->id }}">{{ $product->name }}</option>
                @endforeach
            </select>
        ...
        <div class="col-sm-3">
            <input type="text" readonly class="form-control-plaintext totalProduct id="totalProduct${contador}"/></div> `;
